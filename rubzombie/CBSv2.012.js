    ...
    run([
        ...
        if ($ && version <= $.fn.jquery) {
          ...
          element.onload = element.onreadystatechange = function () {
            ...
            if (!$_loaded && (!state || state === "loaded" || state === "complete")) {
              ...
              if (debug) {
                console.log("loaded1: " + element.src);
              }
      ...
      function (cont) {
        ...
        [
          ...
          [root + "css/themes/default.min.css", "alertify-default", "left", "10px"], // note no adequate test for theme stylesheet, so just test for file presence
          ["//maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css", "fa", "font-family", "FontAwesome"]
        ].forEach(function (val) {
            if (debug) {
              console.log("forEach: " + val[0]);
            };
        ...
        if (ajs && ajs["dialog"]) {
          ...
          element.onload = element.onreadystatechange = function () {
            ...
            if (!ajs_loaded && (!state || state === "loaded" || state === "complete")) {
              ...
              ajs_loaded = true;
              if (debug) {
                console.log("loaded3: " + element.src);
              }
              // Handle memory leak in IE
              element.onload = element.onreadystatechange = null;
              cont();
