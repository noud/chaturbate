...
// http://www.rlc-cams.com/apps/sourcecode/maetipper/?version=&slot=2
			...
			if (loversList.length > 0) {
				...
				for (var i = 0; i < loversList.length; i++){
					...
					if (i < loversList.length) {
						userMsg += '' + (i + 1) + '. ' + loversList[i].name + '  (' + loversList[i].tokens + ' tks this session, ' +loversList[i].totalTokens + 'tks overall. )';
					}
					else {
						userMsg += '' + (i + 1) + '. ---------------------';
					}	    
...
cb.onTip(function (tip) 
{
	...
	if (loversList.length > 0) {
        ...
        isSecond = (username == loversList[1].name);
        isThird = (username == loversList[2].name);
	}
	...
	if (!isMVP && loversList[0].name == username && loversAnnounce) {
		var msg = ':mtlstar ' + loversList[0].name + ' :mtlstar is the new Top Lover with ' + loversList[0].tokens + ' tokens!';
