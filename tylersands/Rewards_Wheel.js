// https://de.chaturbate.com/apps/user_uploads/0/tylersands/
// https://de.chaturbate.com/apps/sourcecode/rewards-wheel/?version=&slot=0

// Tyler's Reward Wheel :kiis
// by al3x009
// admin@autumnrey.com
...
cb.settings_choices = [
    ...
    {name:'multispin_count', type:'int', minValue:0, maxValue:6, defaultValue:3, label: 'How Many Multi-Spins To Allow? (0-6)'},
...
cb.chatNotice('  Reward Wheel v.2 Activated!');
cb.chatNotice('------------- :kiis -------------------');
...
cb.chatNotice('--------------- :kiis -----------------');
cb.chatNotice('      Developed by @AL3X009     ');


function spinWheel() {
	cb.chatNotice('********* Spinning The Wheel! ********');
	var randomnumber = Math.floor(Math.random()*(rewards.length));
	cb.chatNotice('**** The Wheel Lands On : '  + rewards[randomnumber] + ' *********');
	cb.chatNotice('**************************************');
}
...
function advert() {
    cb.chatNotice('Reward Wheel Is ACTIVE! Tip ' + cb.settings.cost_per_spin + ' To Play! Type \'!rewards\' to get a wheel reward listing');
